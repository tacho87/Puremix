...
Components are **reusable .puremix files** that accept props, have their own data loading, and can contain server functions. They are the building blocks of PureMix applications.

### Component Architecture

A complete PureMix component demonstrates the full data flow:

```
Parent Component passes props
         ↓
Child Component Loader receives props (3rd parameter)
         ↓
Template displays data from loader
         ↓
User submits form → Server Function processes action
         ↓
Server Function returns action result
         ↓
Loader receives actionResult (2nd parameter) on re-render
         ↓
Template displays notification/result
```

### Complete Component Example

**File:** `app/components/QuickActionWidget.puremix`

This component demonstrates all four key patterns working together:
...
<loader>
  async function loadQuickActionWidget(request, actionResult, props) {
    ...
    // STEP 3: Load data and compute display values
    const financialData = await DataManager.readData('financial-data.json');
    const currentBalance = financialData.metrics.totalRevenue;

    // STEP 4: Return data to template
    return {
      data: {
        widgetType,
        title,
...
<div class="widget">
  <h3>{loadQuickActionWidget.data.title}</h3>
  <p>Balance: {loadQuickActionWidget.data.formattedBalance}</p>

  <!-- Notification from server function result -->
  {loadQuickActionWidget.data.notification ?
    <div class="notification">
      {loadQuickActionWidget.data.notification.message}
    </div>
    : ''
  }

  <!-- Form calling component server function -->
  <form onsubmit="QuickActionWidget.submitAction">
...
<script server>
  async function submitAction(formData, request) {
    ...
    return {
      success: true,
      message: `Transaction of $${amount} completed`,
      type: 'success'
    };
  }
</script>
...
### Using Components in Parent

```html
<imports>
  import QuickActionWidget from '../components/QuickActionWidget.puremix'
  import MetricsWidget from '../components/MetricsWidget.puremix'
  import UserList from '../components/UserList.puremix'
</imports>

<div class="dashboard">
  <!-- Pass different props for different behaviors -->
  <QuickActionWidget
    widgetType="transfer"
    title="Send Money"
    showBalance={true}
  ...
  <QuickActionWidget
    widgetType="deposit"
    title="Add Funds"
    initialValues={{ source: 'Bank Account' }}
  />

  <!-- Components can also pass data from loader -->
  <MetricsWidget
    metricType="revenue"
    title="Total Revenue"
  ...
  <!-- Compact view with boolean prop -->
  <UserList
    users={loadDashboard.data.users}
    compact={true}
  />
</div>
```
...
Props are passed from parent components and received in the loader's third parameter:

```javascript
// Loader signature
async function loadComponentName(request, actionResult, props) {
  // props contains all attributes passed to the component
}
```
...
<!-- Boolean props -->
<Modal isOpen={true} />
<UserList compact={false} />

<!-- Object props (data from loader) -->
<UserCard user={loadPage.data.user} />

<!-- Array props -->
<TransactionTable transactions={loadPage.data.transactions} />
...
Always provide defaults for props to make components reusable:

```javascript
async function loadMyWidget(request, actionResult, props) {
  // Provide defaults for all props
  const title = props.title || 'Default Title';
  const showIcon = props.showIcon !== false; // default true
  const maxSize = props.maxSize || 100;
  const items = props.items || [];

  return { data: { title, showIcon, maxSize, items } };
}
```

### Component Patterns

#### 1. Conditional Views (Compact/Full)

Pass a `compact` prop to switch between detailed and summary views:

```html
<loader>
  async function loadTransactionTable(request, actionResult, props) {
    const transactions = props.transactions || [];
    const compact = props.compact || false; // default to full view

    // Limit data for compact view
    const displayData = compact ? transactions.slice(0, 5) : transactions;

    return { data: { transactions: displayData, compact } };
  }
</loader>

{loadTransactionTable.data.compact ?
  // Compact view - simple list
  <div class="space-y-2">
    {loadTransactionTable.data.transactions.map(t =>
      <div class="compact-row">{t.description}</div>
    )}
  </div>
:
  // Full view - detailed table
  <table class="w-full">
    {loadTransactionTable.data.transactions.map(t =>
      <tr>
        <td>{t.description}</td>
...
Use props to change colors and icons based on type:

```html
<loader>
  async function loadMetricsWidget(request, actionResult, props) {
    const metricType = props.metricType || 'revenue';
    ...
    return { data: { metricType, bgColor, icon } };
  }
</loader>

<div class={loadMetricsWidget.data.bgColor}>
  <span class="icon">{loadMetricsWidget.data.icon}</span>
  <span>{loadMetricsWidget.data.metricType}</span>
</div>
```

#### 3. List Rendering

Components commonly receive arrays and render them with `.map()`:

```html
<loader>
  async function loadUserList(request, actionResult, props) {
...
<div class="user-list">
  {loadUserList.data.users.map(user =>
...
Parent components compose multiple child components:

```html
<!-- Dashboard.puremix - Master Component -->
<imports>
...
<loader>
  async function loadDashboard(request, actionResult) {
    ...
    return { data: { financialData, users, transactions } };
  }
</loader>

<div class="dashboard grid grid-cols-4 gap-6">
  <!-- Metrics Row -->
  <MetricsWidget metricType="revenue" value={loadDashboard.data.financialData.metrics.totalRevenue} />
  <MetricsWidget metricType="profit" value={loadDashboard.data.financialData.metrics.totalProfit} />
  <MetricsWidget metricType="expenses" value={loadDashboard.data.financialData.metrics.totalExpenses} />
  <MetricsWidget metricType="users" value={loadDashboard.data.users.filter(u => u.isActive).length} />

  <!-- Charts Row -->
  <FinancialChart chartType="line" data={loadDashboard.data.financialData.revenue} />
  <FinancialChart chartType="pie" data={loadDashboard.data.financialData.categories} />

  <!-- Tables Row -->
  <TransactionTable transactions={loadDashboard.data.transactions} compact={true} />
  <UserList users={loadDashboard.data.users} compact={true} />
</div>
```

### Server Functions in Components

Server functions in components are **automatically namespaced** with the component name:

```html
<!-- UserList.puremix -->
<script server>
  async function saveUser(formData, request) {
    // This function is available globally as UserList.saveUser()
    const name = formData.get('name');
    await DataManager.addRecord('users.json', { name });
    return { success: true, message: 'User added' };
  }

  async function toggleStatus(userId, formData, request) {
    // Available as UserList.toggleStatus()
    const user = await DataManager.getRecord('users.json', userId);
    await DataManager.updateRecord('users.json', userId, {
...
**Calling namespaced component functions:**

```html
<!-- From within the component -->
<button onclick="UserList.saveUser">Save</button>
<button onclick="UserList.toggleStatus('{user.id}')">Toggle</button>

<!-- From parent component or other pages -->
<button onclick="UserList.saveUser">Add User via UserList</button>
```

### Action Result Handling

When a server function returns data, the loader receives it as `actionResult`:

```html
<loader>
  async function loadMyComponent(request, actionResult, props) {
    // Check if this is a re-render after an action
    let notification = null;
    if (actionResult) {
      // actionResult contains what the server function returned
      notification = {
        message: actionResult.message,
        type: actionResult.type // 'success' | 'error' | 'warning'
      };
    }
...
<!-- Display notification in template -->
{loadMyComponent.data.notification ?
  <div class={'notification ' + loadMyComponent.data.notification.type}>
    {loadMyComponent.data.notification.message}
  </div>
  : ''
}
```
...
<form onsubmit="MyComponent.processData">
  <button type="submit">Submit</button>
</form>

// 2. Server function receives form data
<script server>
async function processData(formData, request) {
  const result = await someOperation(formData.get('value'));
...
<loader>
async function loadMyComponent(request, actionResult, props) {
  // actionResult now contains:
  // { success: true, message: '...', type: 'success', resultData: ... }
...
Components can include `<script client>` blocks for browser-side interactivity:

```html
<script client>
  document.addEventListener('DOMContentLoaded', function() {
    // Access component data from PureMix.data
    const componentData = PureMix.data?.loadMyComponent?.data;

    if (componentData) {
      console.log('Component loaded with:', componentData);

      // Add client-side behavior
      const form = document.querySelector('form');
      if (form) {
        form.addEventListener('submit', function(e) {
          // Pre-processing before server submission
          const input = form.querySelector('input[name="amount"]');
          if (input && parseFloat(input.value) > 10000) {
            if (!confirm('Large transaction - continue?')) {
              e.preventDefault();
            }
          }
        });
...
### Python Integration in Components

Components can import and call Python functions directly:
...
async function predictRealEstate(formData, request) {
  ...
  const propertyData = {
    ...
    square_meters: parseFloat(formData.get('sqft')),
    rooms: parseInt(formData.get('rooms'))
  };

  // Call Python function like a normal JS function
  const result = await predict_property_price(propertyData);

  if (result.success) {
    return {
      success: true,
      message: `Predicted price: $${result.data.predicted_price.toLocaleString()}`,
      prediction: result.data
    };
  } else {
    return {
      success: false,
      message: 'Prediction failed: ' + result.error,
      type: 'error'
    };
  }
}
...
- Use the third parameter `props` in loader functions
- Handle `actionResult` in loaders for feedback
- Namespace server functions with component names
- Include `<script client>` for progressive enhancement
- Return formatted values from loaders for templates
- Use descriptive component names (PascalCase)

❌ **DON'T:**

- Forget to handle `actionResult` in the loader
- Mix component-scoped functions with global ones
- Return raw data without formatting
- Use inconsistent naming conventions
- Create overly large components (break them down)
